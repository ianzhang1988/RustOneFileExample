...
impl <T: Ord> NodeType<T> {
    ...
    // no idea how to implement delete here, in algo_binary_tree3, use cell to solve the problem, but
    // somewhat much more complicated
    ...
    //                 },
    //                 Ordering::Equal => {
    //                     *self = Null; // assignment to borrowed `*self` occurs here
    //                     // Some((**l,**r)) // move occurs because `**l` has type `NodeType<T>`, which does not implement the `Copy` trait
    //                     None
    //                 }
    //             }
    //         },
